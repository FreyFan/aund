...
function createwindow(title, addurl,width,height) {
    ...
    //--author：Frey.Fan---------date：20140427---------for：弹出bug修改,设置了zindex()函数
	if(typeof(windowapi) == 'undefined'){
		$.dialog({
			content: 'url:'+addurl,
			lock : true,
...
function doSubmit(url,name,data) {
	gridname=name;
	//--author：Frey.Fan ---------date：20140227---------for：把URL转换成POST参数防止URL参数超出范围的问题
	var paramsData = data;
	if(!paramsData){
...
function addOneTab(subtitle, url, icon) {
	var indexStyle = getCookie("PLATFORMINDEXSTYLE");
 ...
 * 代入查询条件
 */
function PlatformExcelExport(url,datagridId){
 ...
 * @returns {Array}
 */
function platformAutoParse(data){
	var parsed = [];
