package com.kd.platform.core.interceptors;
...
import com.kd.platform.core.extend.hqlsearch.SysContextSqlConvert;
import com.kd.platform.core.util.ContextHolderUtils;
import com.kd.platform.core.util.PlatformDataAutorUtils;
...
public class AuthInterceptor implements HandlerInterceptor {
            ...
            if (client != null && client.getUser() != null) {
                ...
                if (!oConvertUtils.isEmpty(functionId)) {
                    ...
                    if (allOperation.size() > 0) {
                        ...
                        String hasOperSql = "SELECT operation FROM PLATFORM.t_s_role_function fun, PLATFORM.t_s_role_user role WHERE  " + "fun.functionid='" + functionId + "' AND fun.operation!=''  AND fun.roleid=role.roleid AND role.userid='" + client.getUser().getId() + "' ";
                    ...
                    PlatformDataAutorUtils.installDataSearchConditon(request, MENU_DATA_AUTHOR_RULE_SQL);// 菜单数据规则sql
                    PlatformDataAutorUtils.installDataSearchConditon(request, MENU_DATA_AUTHOR_RULES);// 菜单数据规则集合
    ...
    private boolean hasMenuAuth(HttpServletRequest request) {
        ...
        if (functionList == null) {
            functionList = this.systemService.findByProperty(TSFunction.class, "functionType", (short) 0);
        }
        ...
        String sql = "SELECT DISTINCT f.id FROM PLATFORM.t_s_function f,PLATFORM.t_s_role_function  rf,PLATFORM.t_s_role_user ru " + " WHERE f.id=rf.functionid AND rf.roleid=ru.roleid AND " + "ru.userid='" + userid + "' AND f.functionurl like '" + requestPath + "%'";
        List list = this.systemService.findListbySql(sql);
        if (list.size() == 0) {
            String orgId = currLoginUser.getCurrentDepart().getId();
            String functionOfOrgSql = "SELECT DISTINCT f.id from PLATFORM.t_s_function f, PLATFORM.t_s_role_function rf, PLATFORM.t_s_role_org ro  " + "WHERE f.ID=rf.functionid AND rf.roleid=ro.role_id " + "AND ro.org_id='" + orgId + "' AND f.functionurl like '" + requestPath + "%'";
