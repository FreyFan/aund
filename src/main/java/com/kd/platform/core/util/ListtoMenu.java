...
/**
 * 动态菜单栏生成
 * @author Frey.Fan 获取是否有子节点不用查询数据库;
 */
public class ListtoMenu {
    ...
    static void iterGet(List<TSFunction> set1, String pid, StringBuffer buffer) {
        ...
        for (TSFunction node : set1) {
            ...
            {
                buffer.append("{\"menuid\":\"" + node.getId() + " \",\"icon\":\"" + TSIcon.allTSIcons.get(node.getTSIcon().getId()).getIconClas() + "\"," + "\"menuname\":\"" + getMutiLang(node.getFunctionName()) + "\",\"url\":\"" + node.getFunctionUrl() + "\"");
                if (count == set1.size()) {
                    buffer.append("}\n");
                }
                buffer.append("},\n");
    ...
    public static String getBootMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            /*
             * int submenusize = pFunction.getSubFunctionSize(); if (submenusize == 0) {
             * menuString.append("</li>"); } if (submenusize > 0) {
             * menuString.append("<ul class=\"sub\">"); } for (TSFunction function : functions) { if
             * (function.getTSFunction().getId().equals(pFunction.getId())) { menuString
             * .append("<li><a href=\"" + function.getFunctionUrl() +
             * "\" target=\"contentiframe\"><span class=\"icon16 icomoon-icon-file\"></span>" +
             * getMutiLang(function.getFunctionName()) + "</a></li>"); } } if (submenusize > 0) {
    ...
    public static String getEasyuiMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
        StringBuffer menuString = new StringBuffer();
        for (TSFunction pFunction : pFunctions) {
            menuString.append("<div  title=\"" + getMutiLang(pFunction.getFunctionName()) + "\" iconCls=\"" + TSIcon.allTSIcons.get(pFunction.getTSIcon().getId()).getIconClas() + "\">");
            /*
             * int submenusize = pFunction.getSubFunctionSize(); if (submenusize == 0) {
             * menuString.append("</div>"); } if (submenusize > 0) { menuString.append("<ul>"); }
             * for (TSFunction function : functions) { if
             * (function.getTSFunction().getId().equals(pFunction.getId())) { String icon =
             * "folder"; if (function.getTSIcon() != null) { icon =
             * TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas(); }
             * menuString.append("<li><div onclick=\"addTab(\'" +
             * getMutiLang(function.getFunctionName()) + "\',\'" + function.getFunctionUrl() +
             * "&clickFunctionId=" + function.getId() + "\',\'" + icon + "\')\"  title=\"" +
             * getMutiLang(function.getFunctionName()) + "\" url=\"" + function.getFunctionUrl() +
             * "\" iconCls=\"" + icon + "\"> <a class=\"" + getMutiLang(function.getFunctionName())
            ...
            for (TSFunction function : functions) {
                if (function.getTSFunction().getId().equals(pFunction.getId())) {
                    ...
                    menuString.append("<li><div onclick=\"addTab(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + function.getFunctionUrl() + "&clickFunctionId=" + function.getId() + "\',\'" + icon + "\')\"  title=\"" + getMutiLang(function.getFunctionName()) + "\" url=\"" + function.getFunctionUrl() + "\" iconCls=\"" + icon + "\"> <a class=\"" + getMutiLang(function.getFunctionName()) + "\" href=\"#\" > <span class=\"icon " + icon + "\" >&nbsp;</span> <span class=\"nav\" >"
                            + getMutiLang(function.getFunctionName()) + "</span></a></div></li>");
                }
            }
            if (hasSubfun) {
                menuString.append("</ul></div>");
    ...
    public static String getEasyuiMultistageMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            menuString.append("<div   title=\"" + getMutiLang(function.getFunctionName()) + "\" iconCls=\"" + TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\">");
            /*
             * int submenusize = function.getSubFunctionSize(); if (submenusize == 0) {
             * menuString.append("</div>"); } if (submenusize > 0) { menuString.append("<ul>"); }
        ...
        for (TSFunction function : list) {
            if (function.getTSFunction().getId().equals(parent.getId())) {
                if (!function.hasSubFunction(map)) {
                    ...
                    menuString.append("<div title=\"" + getMutiLang(function.getFunctionName()) + "\" iconCls=\"" + TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\"><ul>");
    ...
    public static String getWebosMenu(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction pFunction : pFunctions) {
            ...
            menuString.append("{\"id\":\"" + pFunction.getId() + "\",\"name\":\"" + pFunction.getFunctionName() + "\",\"path\":\"" + TSIcon.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath() + "\",\"level\":\"" + pFunction.getFunctionLevel() + "\",");
    ...
    public static String getAceMultistageTree(Map<Integer, List<TSFunction>> map) {
        ...
        for (TSFunction function : list) {
            ...
            /*
             * int submenusize = function.getSubFunctionSize(); if (submenusize == 0) {
             * menuString.append("</a></li>"); } if (submenusize > 0) {
             * menuString.append("<b class=\"arrow\"></b><ul  class=\"submenu\" >"); }
             * menuString.append(getSubMenu(function,1,map)); if (submenusize > 0) {
             * menuString.append("</ul></li>"); }
             */
            if (!function.hasSubFunction(map)) {
