...
public class FormUtil {
    ...
    public static String GetHtml(String parseHtml, String contentData, String action) {
        ...
        for (int f = 0; f < jsonArray.size(); f++) {
            if (jsonArray.getJSONObject(f) == null || "".equals(jsonArray.getJSONObject(f))) continue;
    ...
    private static String GetTextBox(JSONObject item, Map<String, Object> formData, String action) {
        ...
        if (value == null) value = item.getString("value") == null ? "" : item.getString("value").toString();
    ...
    private static String GetTextArea(JSONObject item, Map<String, Object> formData, String action) {
        String script = "";
        if (item.getString("orgrich") != null && "1".equals(item.getString("orgrich").toString())) script = "orgrich=\"true\" ";
        String name = item.getString("name").toString();
    ...
    private static String GetMacros(JSONObject item, Map<String, Object> formData, String action) {
        ...
        if ("view".equals(action)) return value;
        if (value != null) {
    ...
    private static String GetQrcode(JSONObject item, Map<String, Object> formData, String action) {
        ...
        String qrcode_value = "";
        if (item.getString("value") != null) qrcode_value = item.getString("value").toString();
    ...
    private static String GetListctrl(JSONObject item, Map<String, Object> formData, String action) {
        ...
        for (int i = 0; i < tdCount; i++) {
            if (i == tdCount - 1) listTitle.set(i, "操作");
            if ("view".equals(action) && i == tdCount - 1) continue;// 如果是查看最后一列不显示
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            for (int i = 0; i < tdCount; i++) {
                ...
                if (sum != "")// 一次循环计算该列的值
                {
                    // region 计算统计值
                    float tempTdValue = 0;
                    if (SumValueDic.containsKey(i)) tempTdValue = SumValueDic.get(i);
                    try {
                ...
                if (i == tdCount - 1)// 最后一列不显示
                {
                    ...
                    if ("view".equals(action)) {
                        ...
                        else if ("calc".equals(type)) tr += MessageFormat.format("<td><input class=\"input-medium\" type=\"text\" value=\"{0}\" name=\"{1}[]\" {2}></td>", tdValue, tdname, sum);
                    }
                }
