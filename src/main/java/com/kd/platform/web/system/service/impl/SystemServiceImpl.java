...
import com.kd.platform.web.system.dao.PlatformDictDao;
...
/**
 * <b>Application name:</b> SystemServiceImpl.java <br>
 * <b>Application describing: 系统公用业务接口实现</b> <br>
...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
    ...
    public List<DictEntity> queryDict(String dicTable, String dicCode, String dicText) {
        ...
        if (StringUtil.isEmpty(dicTable)) {
            dictList = platformDictDao.querySystemDict(dicCode);
            ...
            dictList = platformDictDao.queryCustomDict(dicTable, dicCode, dicText);
        }
        return dictList;
    }
    ...
    public TSType getType(String typecode, String typename, TSTypegroup tsTypegroup) {
        // TSType actType = commonDao.findUniqueByProperty(TSType.class, "typecode",
    ...
    public String generateOrgCode(String id, String pid) {
        ...
        if (!StringUtils.hasText(pid)) { // 第一级编码
            String sql = "select max(t.org_code) orgCode from PLATFORM.t_s_depart t where t.parentdepartid is null";
            ...
            String sql = "select max(t.org_code) orgCode from PLATFORM.t_s_depart t where t.parentdepartid = ?";
            ...
            if (orgCodeMap.get("orgCode") != null) { // 当前基本有编码时
                ...
                String pOrgCodeSql = "select max(t.org_code) orgCode from PLATFORM.t_s_depart t where t.id = ?";
