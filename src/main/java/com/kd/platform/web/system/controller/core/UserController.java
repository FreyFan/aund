...
import com.kd.platform.core.common.model.json.ComboBox;
import com.kd.platform.core.common.model.json.DataGrid;
import com.kd.platform.core.common.model.json.ValidForm;
import com.kd.platform.core.constant.Globals;
import com.kd.platform.core.enums.SysThemesEnum;
import com.kd.platform.core.extend.hqlsearch.HqlGenerateUtil;
import com.kd.platform.core.util.ListtoMenu;
import com.kd.platform.core.util.PasswordUtil;
import com.kd.platform.core.util.ResourceUtil;
import com.kd.platform.core.util.RoletoJson;
import com.kd.platform.core.util.SetListSort;
import com.kd.platform.core.util.StringUtil;
import com.kd.platform.core.util.SysThemesUtil;
import com.kd.platform.core.util.oConvertUtils;
import com.kd.platform.tag.core.easyui.TagUtil;
import com.kd.platform.tag.vo.datatable.DataTableReturn;
import com.kd.platform.tag.vo.datatable.DataTables;
import com.kd.platform.web.system.manager.ClientManager;
import com.kd.platform.web.system.pojo.base.TSDepart;
import com.kd.platform.web.system.pojo.base.TSFunction;
import com.kd.platform.web.system.pojo.base.TSRole;
import com.kd.platform.web.system.pojo.base.TSRoleFunction;
import com.kd.platform.web.system.pojo.base.TSRoleUser;
import com.kd.platform.web.system.pojo.base.TSUser;
import com.kd.platform.web.system.pojo.base.TSUserOrg;
...
public class UserController extends BaseController {
    ...
    public AjaxJson del(TSUser user, HttpServletRequest req) {
        ...
        if (!user.getStatus().equals(Globals.User_ADMIN)) {
            if (roleUser.size() > 0) {
                // 删除用户时先删除用户和角色关系表
                delRoleUser(user);
                systemService.executeSql("delete from PLATFORM.t_s_user_org where user_id=?", user.getId()); // 删除
    ...
    public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
        ...
        if (StringUtil.isNotEmpty(user.getId())) {
            ...
            systemService.executeSql("delete from PLATFORM.t_s_user_org where user_id=?", user.getId());
    ...
    public String changeStyle(HttpServletRequest request) {
        ...
        // continue;
        // }
        // if(cookie.getName().equalsIgnoreCase("PLATFORMINDEXSTYLE")){
        // indexStyle = cookie.getValue();
        // }
        // if(cookie.getName().equalsIgnoreCase("PLATFORMCSSTHEME")){
    ...
    public AjaxJson saveStyle(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (user != null) {
            ...
            // Cookie cookie4css = new Cookie("PLATFORMCSSTHEME", cssTheme);
            // cookie4css.setMaxAge(3600*24*30);
            // response.addCookie(cookie4css);
            // logger.info("cssTheme:"+cssTheme);
            // }else if("ace".equals(indexStyle)){
            // Cookie cookie4css = new Cookie("PLATFORMCSSTHEME", "metro");
            // cookie4css.setMaxAge(3600*24*30);
            // response.addCookie(cookie4css);
            // logger.info("cssTheme:metro");
            // }else {
            // Cookie cookie4css = new Cookie("PLATFORMCSSTHEME", "");
            // cookie4css.setMaxAge(3600*24*30);
            // response.addCookie(cookie4css);
            ...
            if (StringUtils.isNotEmpty(indexStyle)) {
                Cookie cookie = new Cookie("PLATFORMINDEXSTYLE", indexStyle);
                // 设置cookie有效期为一个月
