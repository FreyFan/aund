package com.kd.platform.web.cgform.service.impl.config.util;
...
import com.kd.platform.web.cgform.exception.DBException;
...
/**
 * 通过hibernate和脚本来处理来同步数据库 对于修改数据库的字段，考虑各种数据库的情况，字段名称全部、类型修改成大写
 */
public class DbTableProcess {
    ...
    public static void createOrDropTable(CgFormHeadEntity table, Session session) throws IOException, TemplateException, HibernateException, SQLException, DBException {
        Template t;
        t = getConfig("/com/kd/platform/web/cgform/engine/hibernate").getTemplate("tableTemplate.ftl");
    ...
    public List<String> updateTable(CgFormHeadEntity table, Session session) throws DBException {
        ...
        try {
            ...
            for (String columnName : cgFormColumnMetaMap.keySet()) {
                if (!dataBaseColumnMetaMap.containsKey(columnName)) {// 表如果不存在该列，则要对表做修改、增加、删除该列动作
    ...
    private static void createTable(String xml, CgFormHeadEntity table, Session session) throws HibernateException, SQLException, DBException {
        ...
        // .setProperty("hibernate.connection.username",propertiesUtil.readProperty("jdbc.username.platform"))
        // .setProperty("hibernate.connection.password",propertiesUtil.readProperty("jdbc.password.platform"))
        // .setProperty("hibernate.dialect",propertiesUtil.readProperty("hibernate.dialect"))
        // .setProperty("hibernate.connection.url",propertiesUtil.readProperty("jdbc.url.platform"))
