package com.kd.platform.web.cgform.controller.generate;
...
import com.kd.platform.codegenerate.generate.onetomany.CgformCodeGenerateOneToMany;
...
import com.kd.platform.web.cgform.entity.generate.GenerateSubListEntity;
import com.kd.platform.web.cgform.service.button.CgformButtonServiceI;
import com.kd.platform.web.cgform.service.button.CgformButtonSqlServiceI;
...
public class GenerateController extends BaseController {
    ...
    public ModelAndView gogenerate(CgFormHeadEntity cgFormHead, HttpServletRequest request) {
        ...
        if (cgFormHead.getJformType() == 1 || cgFormHead.getJformType() == 3) {
            // 如果是单表或者附表，则进入单表模型
            returnModelAndView = "platform/cgform/generate/single";
        } else {
            // 如果是主表，则进入一对多模型
            ...
            if (StringUtil.isNotEmpty(cgFormHead.getSubTableStr())) {
                ...
                for (String subTable : subTables) {
                    ...
                    entityNameMap.put(subHead.getTableName(), PlatformReadTable.formatFieldCapital(subHead.getTableName()));
                }
            }
            request.setAttribute("subTableList", subTableList);
            returnModelAndView = "platform/cgform/generate/one2many";
        }
        ...
        String entityName = PlatformReadTable.formatFieldCapital(cgFormHead.getTableName());
    ...
    public void dogenerate(CgFormHeadEntity cgFormHead, GenerateEntity generateEntity, CreateFileProperty createFileProperty, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            // step.2 判断表是否存在
            boolean tableexist = new PlatformReadTable().checkTableExist(tableName);
            if (tableexist) {
                // step.3 判断是不是用用户自定义界面
                String html;
                CgformCodeGenerate generate = new CgformCodeGenerate(createFileProperty, generateEntity);
                if (createFileProperty.getJspMode().equals("04") && (html = cgformFtlService.getUserFormFtl(cgFormHead.getId())) != null) {
    ...
    private String replaceAddJSP(String cgformJspHtml) {
        ...
        while (cgformJspHtml.indexOf("#{") > 0) {
            ...
            cgformJspHtml = cgformJspHtml.replace(key, "<input id='" + PlatformReadTable.formatField(realKey) + "' name='" + PlatformReadTable.formatField(realKey) + "' type='text' value='${'$'}{${entityName?uncap_first}Page." + PlatformReadTable.formatField(realKey) + "}' style='width: 150px' class='inputxt' >");
        }
    ...
    private void getCgformConfig(CgFormHeadEntity cgFormHead, GenerateEntity generateEntity) throws Exception {
        ...
        for (CgFormFieldEntity field : cgFormHead.getColumns()) {
            ...
            if (listJsCopy != null) {
                listJsCopy.setCgJsStr(listJsCopy.getCgJsStr().replace(fieldName, PlatformReadTable.formatField(fieldName)));
            }
            if (formJsCopy != null) {
                formJsCopy.setCgJsStr(formJsCopy.getCgJsStr().replace(fieldName, PlatformReadTable.formatField(fieldName)));
            }
        }
    ...
    public ModelAndView goFileTree(HttpServletRequest request) {
        return new ModelAndView("platform/cgform/generate/fileTree");
    }
