package com.kd.platform.web.cgform.controller.cgformcategory;
...
import com.kd.platform.web.cgform.entity.category.CgformCategoryEntity;
import com.kd.platform.web.cgform.service.cgformcategory.CgformCategoryServiceI;
...
public class CgformCategoryController {
     ...
     */
    private static final Logger logger = Logger.getLogger(CgformCategoryController.class);

    private static final String CGFORM_CATEGORY_LIST = "platform/cgform/cgformcategory/cgformCategoryList";

    private static final String CGFORM_CATEGORY = "platform/cgform/cgformcategory/cgformCategory";
    ...
    public void datagrid(CgformCategoryEntity cgformCategory, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        CriteriaQuery cq = new CriteriaQuery(CgformCategoryEntity.class, dataGrid);
        HqlGenerateUtil.installHql(cq, cgformCategory);
    ...
    public AjaxJson del(CgformCategoryEntity cgformCategory, HttpServletRequest request) {
        ...
        try {
            cgformCategory = cgformCategoryService.getEntity(CgformCategoryEntity.class, cgformCategory.getId());
            j.setMsg("表单分类管理删除成功");
    ...
    public AjaxJson save(CgformCategoryEntity cgformCategory, HttpServletRequest request) {
        ...
        try {
            if (StringUtil.isNotEmpty(cgformCategory.getId())) {
                j.setMsg("表单分类管理更新成功");
                CgformCategoryEntity t = cgformCategoryService.get(CgformCategoryEntity.class, cgformCategory.getId());
