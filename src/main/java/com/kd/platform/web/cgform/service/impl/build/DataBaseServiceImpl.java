package com.kd.platform.web.cgform.service.impl.build;
...
import com.kd.platform.web.cgform.common.CommUtils;
...
import com.kd.platform.web.cgform.service.build.DataBaseService;
...
import com.kd.platform.core.util.UUIDGenerator;
...
public class DataBaseServiceImpl extends CommonServiceImpl implements DataBaseService {
    ...
    public void executeSqlExtend(String formId, String buttonCode, Map<String, Object> data) {
        ...
        if (cgformButtonSqlVo != null) {
            ...
            if (StringUtils.isNotEmpty(sqlPlugin)) {
                ...
                for (String sql : sqls) {
                    if (sql.toLowerCase().indexOf(CgAutoListConstant.SQL_INSERT) != -1 || sql.toLowerCase().indexOf(CgAutoListConstant.SQL_UPDATE) != -1) {
                        // 执行sql
    ...
    private String replaceExtendSqlSysVar(String sql) {
        sql = sql.replace("{" + DataBaseConstant.SYS_USER_CODE_TABLE + "}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE)).replace("{" + DataBaseConstant.SYS_USER_NAME_TABLE + "}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_NAME)).replace("{" + DataBaseConstant.SYS_ORG_CODE_TABLE + "}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_ORG_CODE))
                .replace("{" + DataBaseConstant.SYS_COMPANY_CODE_TABLE + "}", ResourceUtil.getUserSystemData(DataBaseConstant.SYS_COMPANY_CODE)).replace("{" + DataBaseConstant.SYS_DATE_TABLE + "}", DateUtils.formatDate()).replace("{" + DataBaseConstant.SYS_TIME_TABLE + "}", DateUtils.formatTime());
        return sql;
    }
