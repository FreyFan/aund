 ...
 */
@Controller
@RequestMapping("/smartSocketHistoryAnalyzeController")
public class SmartSocketHistoryAnalyzeController extends BaseController {
    /**
     * Logger for this class
     */
    private static final Logger logger = Logger.getLogger(SmartSocketAnalyzeController.class);
    ...
    public void listAllStatisticByJdbc(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        List<Map<String, Object>> maplist = systemService.findForJdbc("SELECT s.classname classname ,count(className) personcount FROM PLATFORM.T_s_Student s group by s.className", null);
        Long countSutent = systemService.getCountForJdbc("SELECT COUNT(1) FROM PLATFORM.T_S_student WHERE 1=1");
        for (Map map : maplist) {
        ...
        if (JdbcDao.DATABSE_TYPE_SQLSERVER.equals(DBTypeUtil.getDBType())) {
            count = systemService.getCountForJdbcParam("select count(0) from (SELECT s.className  classname ,count(className) totalclass FROM PLATFORM.T_s_Student  s group by s.className) as t( classname, totalclass)", null);
        } else {
            count = systemService.getCountForJdbcParam("select count(0) from (SELECT s.className ,count(className) FROM PLATFORM.T_s_Student s group by s.className)t", null);
        }
    ...
    public ModelAndView studentStatisticTabs(HttpServletRequest request) {
    	//SmartSocketAnalyze  reportDemo  SmartSocketAnalyze
        return new ModelAndView("platform/demo/base/report/SmartSocketHistoryAnalyze");
    }
    ...
    public List<Highchart> studentCount(HttpServletRequest request, String reportType, HttpServletResponse response) {
        ...
        Long count = systemService.getCountForJdbc("SELECT COUNT(1) FROM PLATFORM.T_S_student WHERE 1=1");
